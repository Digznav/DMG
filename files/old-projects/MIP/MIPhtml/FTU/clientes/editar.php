...
switch($_POST["action"]) {
	...
	break;
	case "updEmpresa":
		$datoEmpresa = DAOFactory::getDatosEmpresaDAO()->load( $_POST["idEmpresa"] );
	...
	case "addContacto":
		...
		{
				$contacto->acceso = "S";
				Security::registerUser($contacto->email, $contacto->nombre, "Cliente");
		}
		DAOFactory::getContactoDAO()->insert( $contacto );
	break;
	case "updContacto":

		$contacto = DAOFactory::getContactoDAO()->load( $_POST["idContacto"] );
		...
		if ( $_POST["acceso"] != $contacto->acceso )
		{
			if ( $_POST["acceso"] === "S" ){
				Security::registerUser($contacto->email, $contacto->nombre, "Cliente");
			}
			else{
				DAOFactory::getAccesoDAO()->delete( $contacto->email );	
			}
		}

		$contacto->acceso = $_POST["acceso"];

		DAOFactory::getContactoDAO()->update( $contacto );
	break;
	case "removeContacto":
		DAOFactory::getContactoDAO()->delete( $_POST["idContacto"] );
	break;
} 

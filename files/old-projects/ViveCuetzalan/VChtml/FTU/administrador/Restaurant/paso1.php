...
if( !empty($_POST) && !empty($_FILES) ) {
	...
	$restaurant->fotoHd = 'empty';
	if ($_FILES['logo']['name'] != "" ){
		$folderName = clean($restaurant->nombre);
		$fileName = 'logo'.$restaurant->id.'-'.rand(1,99);
	...
	//echo '<script type="text/javascript">window.location.replace("paso2.php?id='.$restaurant->id.'&fd='.$folderName.'");</script>';
	header('Location: paso2.php?id='.$restaurant->id.'&fd='.$folderName );
	die();
} ?>
<!DOCTYPE html>
	...
	<body>
		<div id="container">
			...
			<div class="cont onlyText clearfix">
				<h2 class="sectionTitle">Añadir restaurant - Datos generales</h2>

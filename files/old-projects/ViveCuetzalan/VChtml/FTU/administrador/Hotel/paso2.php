...
if( !empty($_POST) && !empty($_FILES)) {
	...
	if ($_FILES['imgHD']['name'] != "" ){
		$folderName = $_GET['fd'];
		$fileName = 'imgHD'.$hotel->id.'-'.rand(1,99);
		...
		if ((($_FILES['imgHD']['type'] == 'image/jpeg') || ($_FILES['imgHD']['type'] == 'image/png') || ($_FILES['imgHD']['type'] == 'image/pjpeg')) && in_array($extension, $allowedExts)) {
			if ( $_FILES['imgHD']['error'] == 0 ) {
				...
				if ( !file_exists($uploadFile) ) {
					...
					if ($upload) {
						$hotel->fotoHd = 'imgs/dondehospedarse/'.$folderName.'/'.$fileName.'.'.$extension;
	...
	//echo '<script type="text/javascript">window.location.replace("paso3.php?id='.$hotel->id.'&fd='.$folderName.'");</script>';
	header('Location: paso3.php?id='.$hotel->id.'&fd='.$folderName );
	die();
}
?>
<!DOCTYPE html>
	...
	<body>
		<div id="container">
			...
			<div class="cont onlyText clearfix">
				<h2 class="sectionTitle">Añadir hotel - Imagen de fondo</h2>

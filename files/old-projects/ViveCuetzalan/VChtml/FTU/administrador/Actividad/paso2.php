...
if( !empty($_POST) && !empty($_FILES)) {
	...
	if ($_FILES['imgHD']['name'] != "" ){
		$folderName = $_GET['fd'];
		$fileName = 'imgHD'.$actividad->id.'-'.rand(1,99);
	...
	//echo '<script type="text/javascript">window.location.replace("paso3.php?id='.$actividad->id.'&fd='.$folderName.'");</script>';
	header('Location: paso3.php?id='.$idActividad.'&fd='.$folderName );
	die();
}
?>
<!DOCTYPE html>
	...
	<body>
		<div id="container">
			...
			<div class="cont onlyText clearfix">
				<h2 class="sectionTitle">Añadir actividad - Imagen de fondo</h2>

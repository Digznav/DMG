<?php
$count = 1;
$galeriaActividad = DAOFactory::getFotosActivDAO()->queryByIdActiv( $idActividad );

if ( isset($_GET['delImg']) && $_GET['delImg'] != '' ) {
	DAOFactory::getFotosActivDAO()->delete($_GET['delImg']);
	header('Refresh:1; url=editar.php?id='.$idActividad.'&action=modGallery');
};
...
<ul class="galleryList clearfix">
	<?php foreach ($galeriaActividad as $value) { ?>
	<li class="img-li" data-num-img="<?php echo $count; ?>">
		<a <?php echo 'href="editar.php?id='.$idActividad.'&action=modGallery&delImg='.$value->id.'"'; ?>
		<a href="#" class="deleteImg" data-id="<?php echo $value->id; ?>" data-id-hotel="<?php echo $value->idActiv; ?>" onclick="return confirm('¿Estas seguro de eliminar esta imagen?')">

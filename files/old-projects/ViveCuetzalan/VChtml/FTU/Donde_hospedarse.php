...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	...
	<body>
		<div id="container">
			...
			<?php $urlEng = "en/Where_to_stay"; $actMenu = 'Donde_hospedarse'; include("libs/header.php");
			include("libs/reservation.php");
			$sqlcomida = mysql_query("SELECT * FROM restaurantes")or die(mysql_error());
			$k=0;
			while ($rescomida = mysql_fetch_array($sqlcomida)) {
				$resultcomida[$k] = $rescomida;
				$k++;
			}
			$backs = array();
			foreach ($resultcomida as $key) {
				$img = '{src:"'.$key[4].'",fade:1000}';
			...
			<div class="cont imageText clearfix">
				<ul id="logosGrid">
					<?php
					foreach ($resulthotel as $key) { echo '<li><a href="Hotel?id='.$key[0].'"><img src="'.$key[4].'"><p>'.$key[1].'</p></a></li>';}
					?>
				</ul>
			</div>
		</div>
		...
		<script type="text/javascript">
			var iniBackgrounds = hotelsBackgrounds;
			(function($) {
